 ...
 * The webview may only hold a `csv.maxRows` prefix; computing these client-side
...
export function rejectReplaceAllWhileTruncated(
  ...
  if (ops.some((op) => op.kind === "replaceAll")) {
    return (
      ...
      "Raise csv.maxRows, or use a host-side transform / pipeline so unloaded rows are not dropped."
    );
  }
  return undefined;
}
