...
On a file larger than `csv.maxRows`, statistics cover the loaded rows and the panel says so.
...
The SQL panel runs real SQLite queries over the file through sql.js, which is SQLite compiled to WebAssembly. The whole file is loaded into a table named `csv`, whatever `csv.maxRows` is set to.
...
- Results are capped by `csv.sqlResultLimit`, which defaults to 5,000 rows. The panel tells you when a result was truncated.
